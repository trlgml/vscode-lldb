...
class Extension
  ...
  constructor(context: ExtensionContext) {
    ...
    subscriptions.push(
      debug.registerDebugAdapterDescriptorFactory('lldb', this),
    )

    subscriptions.push(
      commands.registerCommand('lldb.diagnose', () => this.runDiagnostics()),
    )
    subscriptions.push(
      commands.registerCommand('lldb.getCargoLaunchConfigs', () =>
        this.getCargoLaunchConfigs(),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.pickMyProcess', () =>
        pickProcess(context, false),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.pickProcess', () =>
        pickProcess(context, true),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.changeDisplaySettings', () =>
        this.changeDisplaySettings(),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.attach', () => this.attach()),
    )
    subscriptions.push(
      commands.registerCommand('lldb.alternateBackend', () =>
        this.alternateBackend(),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.commandPrompt', () =>
        this.commandPrompt(),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.symbols', () =>
        pickSymbol(debug.activeDebugSession),
      ),
    )
    subscriptions.push(
      commands.registerCommand('lldb.viewMemory', () => this.viewMemory()),
    )

    subscriptions.push(
      workspace.onDidChangeConfiguration((event) => {
        if (
          event.affectsConfiguration('lldb.displayFormat') ||
    ...
    this.registerDisplaySettingCommand(
      'lldb.toggleConsoleMode',
      async (settings) => {
        settings.consoleMode =
          settings.consoleMode == 'commands' ? 'evaluate' : 'commands'
      },
    )
    this.registerDisplaySettingCommand(
      'lldb.showDisassembly',
      async (settings) => {
        settings.showDisassembly = <AdapterSettings['showDisassembly']>(
          await window.showQuickPick(['always', 'auto', 'never'])
        )
      },
    )
    this.registerDisplaySettingCommand(
      ...
      async (settings) => {
        settings.showDisassembly =
          settings.showDisassembly == 'auto' ? 'always' : 'auto'
      },
    )
    this.registerDisplaySettingCommand(
      'lldb.displayFormat',
      async (settings) => {
        settings.displayFormat = <AdapterSettings['displayFormat']>(
          await window.showQuickPick(['auto', 'hex', 'decimal', 'binary'])
        )
      },
    )
    this.registerDisplaySettingCommand(
    ...
    subscriptions.push(
      debug.onDidChangeActiveDebugSession((session) => {
        if (session && session.type == 'lldb') this.status.show()
  ...
  async onActivate() {
    ...
    if (
      currVersion != lastVersion &&
      !lldbConfig.get('suppressUpdateNotifications')
    ) {
      ...
      if (lastVersion != undefined) {
        ...
        let choice = await window.showInformationMessage(
          'CodeLLDB extension has been updated',
          ...buttons,
        )
        if (choice === buttons[0]) {
          let changelog = path.join(this.context.extensionPath, 'CHANGELOG.md')
          let uri = Uri.file(changelog)
          await commands.executeCommand('markdown.showPreview', uri, null, {
            locked: true,
          })
    ...
    try {
      ...
      if (uri.path == '/launch') {
        ...
        for (let frag of frags) {
          ...
          if (pos > 0) env[frag.substr(0, pos)] = frag.substr(pos + 1)
        }
  ...
  startRpcServer() {
    ...
    if (rpcOptions) {
      ...
      this.rpcServer.processRequest = async (request) => {
        ...
        debugConfig.name = debugConfig.name || debugConfig.program
        if (rpcOptions.token) {
          if (debugConfig.token != rpcOptions.token) return ''
  ...
  registerDisplaySettingCommand(
    command: string,
    updater: (settings: AdapterSettings) => Promise<void>,
  ) {
    ...
    qpick.items = [
      {
        ...
        detail:
          'Default format for displaying variable values and evaluation results.',
      ...
      {
        label: `Dereference pointers: ${
          settings.dereferencePointers ? 'on' : 'off'
        }`,
        detail:
          'Whether to show a summary of the pointee or a numeric pointer value.',
        command: 'lldb.toggleDerefPointers',
      },
      {
        label: `Console mode: ${settings.consoleMode}`,
        detail:
          'Whether Debug Console input is treated as debugger commands or as expressions to evaluate.',
  ...
  async resolveDebugConfiguration(
    ...
    if (!(await this.checkPrerequisites(folder))) return undefined
    ...
    launchConfig.relativePathBase =
      launchConfig.relativePathBase || workspace.rootPath
  ...
  async createDebugAdapterDescriptor(
    ...
    let connector = new ReverseAdapterConnector(session)
  ...
  mergeWorkspaceSettings(
    launchConfig: WorkspaceConfiguration,
    ...
    let mergeConfig = (key: string, reverse: boolean = false) => {
      ...
      let value = !reverse
        ...
        : mergeValues(value2, value1)
      if (!util.isEmpty(value)) debugConfig[key] = value
    }
  ...
  async startDebugAdapter(
    ...
    adapterProcess.on('exit', async (code, signal) => {
      output.appendLine(
        `Debug adapter exit code=${code} (0x${code.toString(
          16,
        )}), signal=${signal}.`,
      )
      if (code != 0) {
  ...
  async getAdapterDylibs(config: WorkspaceConfiguration): Promise<[string]> {
    if (!this.adapterDylibsCache) {
      ...
      if (liblldb) {
        ...
        liblldb = await adapter.findLibLLDB(
  ...
  async runDiagnostics(folder?: WorkspaceFolder) {
    ...
    if (succeeded) {
      ...
      window.showErrorMessage(
        'LLDB self-test has failed.  Please check log output.',
      )
      output.show()
    }
  }
  ...
  async alternateBackend() {
    ...
    box.onDidAccept(async () => {
      try {
        ...
        let message =
          err.code == 'ENOENT' ? `could not find "${err.path}".` : err.message
        await window.showErrorMessage(
          `Failed to query LLDB for library location: ${message}`,
          { modal: true },
        )
        box.show()
      }
  ...
  commandPrompt() {
    ...
    let terminal = window.createTerminal({
      ...
      shellArgs: [
        ...
        'command script import ' + consolePath,
      ],
